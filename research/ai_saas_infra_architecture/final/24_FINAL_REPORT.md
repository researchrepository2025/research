...
3. **Self-managed Kubernetes is a declining minority concentrated in the largest companies.** Overall adoption is estimated at 10-20% of AI SaaS companies, with 22-33% among $200M+ ARR companies (Salesforce confirmed self-managed; Databricks confirmed hybrid; OpenAI and HubSpot classification uncertain). The self-managed share is declining at 3-5 percentage points per year as managed K8s providers add GPU scheduling and ultra-scale cluster support. (Confidence: C:3-5 depending on tier)
...
6. **Published migration case studies exclusively document moves toward K8s, but this reflects severe publication bias and cannot be interpreted as a market-wide pattern.** Three recent (2024-2025) engineering blog disclosures document migrations toward K8s (Figma: ECS to EKS, Grammarly: EC2 to EKS, Salesforce Data Cloud: EC2 to K8s) and one long-standing (2017+) case (HubSpot). Zero counter-migrations are documented, though companies that stayed on or migrated away from K8s almost never publish about it.
...
- Agent 01: CNCF Annual Surveys (2021-2025, n=750-3,800)
- Agent 02: Analyst Reports (Dynatrace, Datadog, Gartner, Flexera, Red Hat)
- Agent 03: Job Posting Analysis (K8s demand, platform engineering trends)
- Agent 04: Engineering Blog Disclosures (OpenAI, Databricks, Figma, Grammarly, Anthropic, etc.)
- Agent 05: Cloud Vendor Case Studies (AWS, Azure, GCP marketing materials)
- Agent 06: StackShare/GitHub Data (technology adoption, open-source signals)
- Agent 07: SEC Filings and Earnings Calls (public company infrastructure disclosures)
- Agent 08: VC/Startup Database Analysis (stage-specific guidance, funding patterns)
...
Every major data source in the analysis carries pro-K8s selection, survivorship, or publication bias. No countervailing source over-represents serverless, PaaS, or ECS/Fargate users. The primary calibration tool is the CNCF-Gartner gap: CNCF reports 80-82% K8s production adoption among its self-selected respondents, while Gartner reports 54% among a broader enterprise sample -- a 26-28 percentage point gap attributable to selection bias. The final estimates apply a 15-28pp discount range to CNCF-anchored figures, with the specific discount varying by cell based on the availability of non-CNCF corroboration.
...
- **<$10M ARR (60-80%, C:4):** The dominant pattern for early-stage AI SaaS. Multiple independent VC advisory sources (Maven Solutions, SaaStr, YC batch patterns) recommend serverless and PaaS for companies with 5-30 engineers. CNCF survey data excludes companies under 500 employees entirely, and at 500-1,000 employees only 9% are K8s users. The range is wide because of bimodality within the tier: pre-revenue/seed companies (0-$2M) are nearly 100% non-K8s, while $5-10M companies begin adopting K8s at meaningful rates.
...
- **$200M+ ARR (12-22%, C:4):** The lowest share, but still non-trivial. Agent 09 estimated 5-15% (primary); Agent 15 estimated 40-55% (any use). The resolution at 12-22% for "primary or significant" blends both sources but leans toward the lower end, recognizing that named $200M+ companies (OpenAI, Databricks, Anthropic, Snowflake, Figma, Grammarly) overwhelmingly use K8s as primary. The Agent 09 range (5-15%) is not discarded -- it is partially reflected in the lower bound. The 40-55% "any use" figure from Agent 15 remains relevant as a measure of complementary serverless/PaaS alongside K8s at this tier.

**Key supporting evidence:**
- Datadog telemetry: 65% of AWS customers use Lambda (any invocation), 70% of GCP customers use Cloud Run
- CNCF 2024: Serverless as primary computing framework declined to 11% (down from 22% in 2022)
- CNCF 2024 NA: 70% of North American enterprises run production serverless workloads
...
- **$200M+ ARR (52-63%, C:6):** The strongest cell in the matrix. Named companies: Anthropic (GKE), Snowflake (EKS), Figma (EKS, migrated from ECS in 2024), Grammarly (EKS, migrated from EC2 in 2025), Notion (EKS), Cohere (OKE), Contextual AI (GKE), Stacks (GKE Autopilot). Supported by CNCF/Dynatrace convergence and SEC filing infrastructure signals. Confidence reduced from draft C:7 to final C:6 because even these named companies are predominantly $1B+ ARR (not representative of $200M-500M companies) and no source directly measures AI SaaS managed K8s prevalence.

**Vendor market share (among managed K8s users):**
- EKS: ~39% share (CNCF 2024 survey), strongest in US
- GKE: ~32% share, strong in AI/ML due to TPU integration
- AKS: ~28% share among 5,000+ employee organizations, inferred strength in EU enterprise
...
**Trend evidence:**
- Cloud-hosted K8s clusters rose from 45% (2022) to 67% (2025)
- Managed K8s share: 61-73% across multiple sources (up from lower levels historically)
- EKS added 100K node support, GKE added 130K-node clusters, CNCF introduced AI Conformance certification -- all reducing the need to self-manage
...
- **$200M+ ARR (22-33%, C:5):** Supported by named examples, though the evidence is thinner than initially reported. After Agent 19's source verification:
  - Salesforce: **Confirmed** self-managed (bare metal K8s)
  - Databricks: **Confirmed** hybrid (mix of self-managed and managed K8s across EKS, AKS, GKE)
  - OpenAI: **Likely** self-managed historically (7,500-node cluster on Azure with custom CNI), but **current state uncertain** given $250B Azure commitment (may have moved to AKS)
  - HubSpot: **Classification unclear** (CNCF case study from 2017-2018, predates EKS maturity; current managed vs self-managed status unknown)
...
- Near-0% any K8s
- Driven by: minimal engineering teams (2-10 people), VC guidance to avoid K8s overhead, managed API usage for AI capabilities
...
**Important caveat:** EU estimates rest on the weakest evidence in the entire analysis (C:2 confidence for all EU cells). Only 2 EU AI company case studies exist in the Wave 1 dataset (Stacks in Amsterdam on GKE Autopilot; Medigold Health in UK on Azure). All EU-specific percentages should be treated as directional hypotheses.
...
| Managed K8s | 45-55% (C:4) | 35-50% (C:2) | Uncertain | AKS may have stronger EU enterprise position due to Microsoft relationships; hyperscaler sovereign cloud offerings (AWS European Sovereign Cloud) partially address sovereignty concerns |
| Self-managed K8s | 8-16% (C:3) | 18-30% (C:2) | EU likely higher | GDPR sovereignty requirements; on-premises deployment demands; EU-native infrastructure options (OVHcloud, Hetzner); Gaia-X sovereignty frameworks |

**Note on EU data quality:** The CNCF regional data showing "82% EU vs 70% Americas in cloud-native development" measures cloud-native development intensity broadly (including containers, microservices, DevOps practices), NOT Kubernetes adoption specifically and NOT AI SaaS companies. This figure supports the claim that EU is more cloud-native in general, but the leap to "higher K8s adoption" is an inference, not a direct data point.
...
2. **Managed K8s share anchored on range, not single point.** The draft anchored on Dynatrace 73% managed K8s; the final report uses the 61-73% range from three partially conflicting sources (Dynatrace 73%, Tigera 61%, StackShare 67%).

3. **Sub-$10M managed K8s confidence reduced from C:5 to C:3.** This was the largest single confidence correction, driven by the recognition that CNCF data excludes companies under 500 employees and VC stage guidance actively recommends against K8s.
...
5. **Self-managed K8s named-company evidence reclassified.** Only 2 of 4 named companies (Salesforce, Databricks) are clearly confirmed as self-managed. OpenAI's current state is uncertain; HubSpot's classification is ambiguous.

6. **Migration evidence reframed.** Changed from "four independent engineering blog disclosures" to "three recent (2024-2025) and one long-standing (2017+)" to reflect that HubSpot's case dates to 2017-2018. Publication bias caveat strengthened.
...
10. **CNCF sample size decline flagged.** The CNCF 2024 survey had n=750 respondents, down from n=988 in 2023 and n=3,800 in 2021, potentially amplifying selection bias.
...
2. **CNCF survey selection bias is large and compounding.** CNCF surveys are answered by self-selected cloud-native practitioners, inflating K8s adoption by an estimated 26-28 percentage points versus Gartner's broader sample. The CNCF 2024 survey's sample declined to n=750 (from n=3,800 in 2021), potentially further concentrating the respondent pool among active K8s users.

3. **Companies under 500 employees are invisible in survey data.** CNCF data stops at 500-1,000 employees (9% of K8s users). Most AI SaaS companies under $50M ARR have fewer than 500 employees. Sub-$10M and $10-50M tier estimates are extrapolated downward from enterprise data, not measured.
...
6. **EU data is extremely sparse.** Only 2 EU AI company case studies exist (Stacks on GKE, Medigold on Azure). All EU-specific estimates should be treated as speculative. The EU AI Act (effective August 2, 2026) may further differentiate EU patterns but no compliance-driven architecture data exists yet.
...
10. **Temporal snapshot risk.** All estimates reflect 2024-2025 data, with some key sources dating to 2021-2023. AI infrastructure evolves rapidly. The Dynatrace 73% managed K8s figure is from 2023 (3 years old); the CNCF managed K8s provider breakdown is from 2021 (5 years old).
...
2. **Datadog or New Relic telemetry segmented for AI/ML companies.** These platforms already provide serverless and K8s adoption metrics. Vertical segmentation for AI SaaS would provide observability-grade data quality for 15+ cells.
...
6. **ECS/Fargate adoption rate for AI workloads.** No equivalent to the Lambda 65% figure exists. Datadog could likely produce this from existing telemetry.
...
9. **CoreWeave/GPU cloud classification study.** Whether K8s-native GPU clouds are managed or self-managed shifts self-managed estimates by 5-10pp.
...
|---|---|---|
| 01_cncf_survey.md | K8s 80% production (DP 1), 56% multi-cloud (DP 9), 66% GenAI on K8s (DP 21), 44% no AI on K8s (DP 22), 11% serverless framework (DP 31), managed K8s 61% (DP 37), 9% at 500-1K employees (DP 27), 82% EU cloud-native (DP 15), n=750 2024 survey (DP 1) | 09-17 |
| 02_analyst_reports.md | Dynatrace 73% managed (DP 1), 63% managed all-envs (DP 2), Gartner 54% K8s (DP 6), 66% GenAI on K8s (DP 8), AI hosting 37/25/13% split (DP 11), Lambda 65% AWS (DP 14), Cloud Run 70% GCP (DP 15), 7% AI containerized workloads (DP 43) | 09-17 |
| 03_job_postings.md | K8s 28% of DevOps roles (DP 2), platform eng 8.35% (DP 3), 73% managed clusters (DP 4), 91% K8s at 1K+ employees (DP 7), 66-70% North America (DP 9), K8s salary $167K (DP 16) | 09-17 |
| 04_tech_blogs.md | Figma ECS-to-EKS (Aug 2024), Grammarly EC2-to-EKS (Jan 2025), OpenAI 7,500 nodes, Databricks hybrid K8s, Salesforce bare metal K8s, HubSpot K8s (2017+), Anthropic GKE, Cohere OKE, Snowflake EKS, Notion EKS, Stacks GKE Autopilot | 09-17 |
| 05_cloud_vendor_cases.md | EKS AI cases (CS 1-4), ECS/Fargate SaaS (CS 10-12), AKS cases (CS 16-17), Medigold UK Azure (CS 20), GKE cases (CS 21-22), OpenAI $250B Azure commitment, EKS 100K node support | 09-17 |
| 06_stackshare_github.md | K8s 96% enterprise adoption, 67% cloud-hosted (up from 45% in 2022), GKE 32% adoption, AKS 28% at 5K+ employees, 70% NA serverless disconnect vs 11% global, GPU support comparison, 77% GitOps adoption | 09-17 |
| 07_sec_earnings.md | AWS $128.7B revenue, 38% IaaS share, AI SaaS 40-50% COGS, traditional SaaS 6-12% hosting, AI gross margins 50-65%, OpenShift $1.9B ARR at 30%+ growth, GPU 10% to 14% of EC2, 83% idle container costs | 09-17 |
| 08_vc_startup_db.md | Seed-stage K8s avoidance guidance, Maven Solutions infra recommendations, YC 66% AI batch, YC 58% Azure credits, CoreWeave ($12B OpenAI contract, 250K GPUs, 62% cost advantage), Bessemer 25% early-stage margins, $37B GenAI enterprise spending 2025 | 09-17 |

### Primary Sources (Underlying Wave 1 Data)

**Surveys and Telemetry:**
- CNCF Annual Surveys 2021-2024/2025 (n=750-3,800)
- Datadog State of Containers and Serverless 2025 (production telemetry)
...
|---|---|---|---|
| X1 | CNCF survey respondents over-represent K8s adopters relative to AI SaaS population (26-28pp measured gap vs Gartner) | Non-K8s adoption is higher than estimated by 10-20pp; K8s adoption is lower | Medium |
...
| X6 | Companies not running AI/ML on K8s (44% per CNCF) include meaningful share using non-K8s cloud-native | If most use VMs, cloud-native non-K8s is overestimated | Medium |
...
| ID | Assumption | Impact if Wrong | Confidence |
|---|---|---|---|
| CN1 | Serverless adoption decline (22% to 11% CNCF) reflects declining primary-framework use, not total use | Cloud-native non-K8s estimates too high | Medium |
...
| MK1 | Managed K8s share is 61-73% (range from Dynatrace, Tigera, StackShare -- not single-point 73%) | Vendor share estimates change | Medium-High |
...
| MK3 | Multi-cloud K8s adoption among AI SaaS higher than general 56% figure due to GPU scarcity | If not more multi-cloud, 20-30% multi-cloud K8s estimate inflated | Medium |
...
| OK2 | OpenShift's $1.9B ARR is proxy for enterprise self-managed K8s demand | OpenShift includes managed offerings (ROSA, ARO) | Medium |
| OK3 | K8s-native GPU clouds (CoreWeave) are hybrid managed/self-managed category | Classification choice shifts self-managed estimates by 5-10pp | Medium |
...
| T3 | VC-backed stage guidance reflects actual startup behavior | If companies adopt K8s early, managed K8s at <$10M could be 25-40% | Low-Medium |
...
| G2 | The 70% NA serverless figure and 11% CNCF global figure measure different things | If same thing, one figure is significantly wrong | High |
...
| G4 | EU enterprise cloud adoption at 45.2% (Eurostat 2023) has not dramatically changed | If adoption accelerated, EU cloud gap may be smaller | Medium |
| G5 | US hyperscaler sovereign offerings are partial solutions (CLOUD Act issue persists) | If resolved, push toward EU-native infrastructure weakens | Medium-High |
| G6 | Stacks (Amsterdam, GKE Autopilot) is not representative of all EU AI SaaS | If representative, EU startup patterns mirror US managed K8s | Low |
...
- CNCF K8s production 80%: 2024 survey
- Dynatrace 73% managed: 2023 (3 years old)
- Gartner 54% K8s: 2025
- StackShare 67% cloud-hosted: 2025
- Tigera 61% managed: 2024
- CNCF provider market share: 2021 (5 years old)
...
|---|---|---|---|---|
| 19-1 | Two distinct "managed K8s" percentages conflated (73% Dynatrace vs 61% Tigera vs 67% StackShare) | Use range 61-73% instead of anchoring on 73%; widen managed K8s overall from 45-55% to 42-55% | **ACCEPTED** | The final report uses 61-73% managed share range and widens overall managed K8s to 40-53% (incorporating bias adjustment) |
| 19-2 | CNCF 2024 sample size decline (n=750, down from n=3,800 in 2021) not flagged | Add note to limitations section; widen CNCF bias discount from 15-25pp to 15-30pp | **ACCEPTED** | Added to limitations. CNCF bias discount range widened to 15-28pp (reflecting the measured 26-28pp CNCF-Gartner gap) |
...
| 19-4 | HubSpot K8s migration dated to 2017, not 2024-2025 | Change "four independent engineering blog disclosures" to clarify temporal distinction | **ACCEPTED** | Reframed as "three recent (2024-2025) and one long-standing (2017+)" |
| 19-5 | CNCF "82% EU vs 70% Americas" measures cloud-native development intensity, not K8s adoption | Clarify the supporting argument for EU K8s adoption | **ACCEPTED** | Clarified in Section 6.2 that CNCF regional data measures cloud-native broadly, not K8s specifically |
| 19-6 | Dynatrace 73% is from 2023 -- age not adequately flagged | Use 61-73% range instead of single-point 73% | **ACCEPTED** | Incorporated via range-based approach (see 19-1) |
| 19-7 | "44% don't run AI/ML on K8s" (CNCF) data point underweighted | More prominently weight in <$10M and $10-50M tier estimates | **PARTIALLY ACCEPTED** | Referenced in managed K8s tier estimates. However, the 44% figure is from CNCF respondents (already K8s-biased), so it is used as a signal rather than an anchor. The recommendation to reduce $10-50M managed K8s by 5-10pp was partially applied (range adjusted from 50-65% to 50-62%) |
| 19-8 | OpenAI "7,500 nodes" classification as self-managed is ambiguous | Reclassify evidence: Salesforce confirmed, Databricks confirmed hybrid, OpenAI likely but uncertain, HubSpot unclear | **ACCEPTED** | Reclassified in Section 5.3. Confidence for $200M+ self-managed reduced from C:6 to C:5 |
...
| 20-10 | Managed K8s $10-50M range inconsistency with intersection logic | Apply intersection (55-65%) per stated methodology, or update notes | **PARTIALLY ACCEPTED** | Final estimate: 50-62%. The intersection logic was partially applied but further adjusted downward (from 55-65%) to account for Agent 19's concerns about API-wrapper companies and CNCF bias. The cell notes explain this compound adjustment |
...
| 21-2 | Sub-$10M estimates contradicted by VC stage guidance | Lower managed K8s <$10M to 12-25%; raise cloud-native to 65-80% | **ACCEPTED** | Final estimates: managed K8s <$10M at 15-28%, cloud-native <$10M at 60-80%. The lower bound for managed K8s (15%) is slightly above Agent 21's 12% because we account for some $5-10M ARR companies genuinely adopting managed K8s |
...
|---|---|---|---|
| Datadog (02_analyst_reports.md, DP 14) | 65% of AWS customers use Lambda | Any Lambda invocation across all AWS customers | Direct (telemetry) |
| Datadog (02_analyst_reports.md, DP 15) | 70% of GCP customers use Cloud Run | Any Cloud Run usage across all GCP customers | Direct (telemetry) |
| CNCF 2024 (01_cncf_survey.md, DP 31) | 11% use serverless computing frameworks | Serverless as primary computing framework among CNCF respondents | Direct (survey) |
| CNCF 2023 (01_cncf_survey.md, DP 32) | 13% (down from 22% in 2022) | Serverless as primary computing framework | Direct (survey) |
| CNCF 2024 NA (06_stackshare_github.md) | 70% of NA enterprises run production serverless | Any production serverless use among North American enterprises | Direct (survey) |

**Resolution:** These figures are not contradictory. They measure different things:
- **65-70% (Datadog):** "Any use at all" -- including event triggers, background processing, CI/CD. Broadest measure.
- **70% (CNCF NA):** "Production loads on serverless" in North America -- broader than primary, narrower than any use.
- **11-13% (CNCF global):** "Serverless as primary computing framework" -- narrowest measure.
